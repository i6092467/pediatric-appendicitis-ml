# This script validates models for predicting the diagnosis.
...
var_names_full <- c("Age", "BMI", "Sex", "Height", "Weight", 
                    ...
                    "Meteorism", "Enteritis")
# Uncomment an appropriate feature set
# Full model
...
#               "Nausea", "AppetiteLoss", "BodyTemp", "WBCCount", "NeutrophilPerc", #"AbdominalGuarding", (kicked out!)
#               "KetonesInUrine", "ErythrocytesInUrine", "WBCInUrine", "CRPEntry",
...
summary(app.data.dis$DiagnosisByCriteria)
...
# Fit the model
logreg.dis <- glm(DiagnosisByCriteria ~ ., data = app.data.dis.imputed, family = "binomial")
summary(logreg.dis)

# Look at the training set performance
pred <- prediction(predict(logreg.dis, type = "response"), app.data.dis.imputed$DiagnosisByCriteria)
...
fold_membership <- sample(c(rep(1 : K, floor(nrow(app.data.dis) / K)),  1 : (nrow(app.data.dis) %% K)), size = nrow(app.data.dis), replace = FALSE)
cm_overall <- NA
for (k in 1 : K) {
  ...
  logreg.k <- glm(DiagnosisByCriteria ~ ., data = d.k.imputed, family = "binomial")
  ...
  pred <- prediction(predict(logreg.k, d.test.imputed, type = "response"), d.test.imputed$DiagnosisByCriteria)
  perf <- performance(pred, "tpr", "fpr")
  ...
  cm_test <- confusionMatrix(factor(as.numeric(predict(logreg.k, d.test.imputed, type = "response") >= 0.5) + 1, levels = c(1, 2), labels = c("appendicitis", "noAppendicitis")), 
                             factor(as.numeric(d.test.imputed$DiagnosisByCriteria), levels = c(1, 2), labels = c("appendicitis", "noAppendicitis")))
...
oldpars <- par()
par(mar = c(15, 5, 5, 5))
barplot(rf.dis$importance[, 3], las = 2)
par(mar = oldpars$mar)

# Variability of variable importance values
set.seed(1799)
B <- 300
imps_b <- matrix(nrow = B, ncol = ncol(app.data.dis.imputed) - 1)
colnames(imps_b) <- row.names(rf.dis$importance)
for (b in 1:B) {
...
oldpars <- par()
jpeg("varimp_dis.jpg", width = 2200, height = 1300, 
     pointsize = 32)
par(mar = c(15, 5, 2, 2))
...
# Look at the training set performance
pred <- prediction(predict(rf.dis, type = "prob")[, 2], app.data.dis.imputed$DiagnosisByCriteria)
perf <- performance(pred, "tpr", "fpr")
# ROC curve
plot(perf, colorize = TRUE, asp = 1)
abline(a = 0, b = 1, col = "gray", lty = 2)
...
fold_membership <- sample(c(rep(1 : K, floor(nrow(app.data.dis) / K)),  1 : (nrow(app.data.dis) %% K)), 
                          size = nrow(app.data.dis), replace = FALSE)
cm_overall <- NA
rocs <- list()
prcs <- list()
prcs_rand <- list()
for (k in 1 : K) {
  ...
  d.k.imputed <- kNN(d.k[, -which(colnames(d.k) == "DiagnosisByCriteria")])[, 1 : (ncol(d.k) - 1)]
  d.k.imputed$DiagnosisByCriteria <- d.k$DiagnosisByCriteria
  rf.k <- randomForest(DiagnosisByCriteria ~ ., data = d.k.imputed)
  d.test <- app.data.dis[!train_flags, ]
  d.test.imputed <- kNN(d.test[, -which(colnames(d.test) == "DiagnosisByCriteria")])[, 1 : (ncol(d.test) - 1)]
  d.test.imputed$DiagnosisByCriteria <- d.test$DiagnosisByCriteria
  pred <- prediction(predict(rf.k, d.test.imputed, type = "prob")[, 2], d.test.imputed$DiagnosisByCriteria)
  perf <- performance(pred, "tpr", "fpr")
  rocs[[k]] <- perf
  auc_test <- performance(pred, measure = "auc")@y.values[[1]]
  perf <- performance(pred, measure = "prec", x.measure = "rec")
  ...
  # Random classifier
  pred_rand <- prediction(sample(predict(rf.k, d.test.imputed, type = "prob")[, 2]), d.test.imputed$DiagnosisByCriteria)
  perf <- performance(pred_rand, measure = "prec", x.measure = "rec")
  prcs_rand[[k]] <- perf
  cm_test <- confusionMatrix(predict(rf.k, d.test.imputed, type = "response"), d.test.imputed$DiagnosisByCriteria)
...
resp <- factor(as.numeric(app.data.dis.imputed$DiagnosisByCriteria), levels = c(1, 2), labels = c("appendicitis", "no appendicitis"))
plot(resp ~ app.data.dis.imputed$AppendixDiameter, xlab = "Appendix Diameter, mm", ylab = "Diagnosis")
plot(resp ~ app.data.dis.imputed$Peritonitis, xlab = "Peritonitis", ylab = "Diagnosis")
plot(resp ~ app.data.dis.imputed$AppendixOnSono, xlab = "Appendix visible in US?", ylab = "Diagnosis")
...
# Fit the model
ntr <- 100
gb.dis <- gbm(as.numeric(DiagnosisByCriteria)-1 ~ ., data = app.data.dis.imputed, n.trees = ntr)

# Examine variable importance
s <- summary.gbm(gb.dis, plotit = FALSE)
oldpars <- par()
par(mar = c(15, 5, 5, 5))
...
# Look at the training set performance
pred <- prediction(predict(gb.dis, type = "response", n.trees = ntr), app.data.dis.imputed$DiagnosisByCriteria)
perf <- performance(pred, "tpr", "fpr")
# ROC curve
plot(perf, colorize = TRUE, asp = 1)
abline(a = 0, b = 1, col = "gray", lty = 2)
...
for (k in 1 : K) {
  ...
  d.k.imputed <- kNN(d.k[, -which(colnames(d.k) == "DiagnosisByCriteria")])[, 1 : (ncol(d.k) - 1)]
  d.k.imputed$DiagnosisByCriteria <- d.k$DiagnosisByCriteria
  gb.k <- gbm(as.numeric(DiagnosisByCriteria)-1 ~ ., data = d.k.imputed, n.trees = ntr)
  d.test <- app.data.dis[!train_flags, ]
  d.test.imputed <- kNN(d.test[, -which(colnames(d.test) == "DiagnosisByCriteria")])[, 1 : (ncol(d.test) - 1)]
  d.test.imputed$DiagnosisByCriteria <- d.test$DiagnosisByCriteria
  pred <- prediction(predict(gb.k, d.test.imputed, type = "response", n.trees = ntr), d.test.imputed$DiagnosisByCriteria)
  perf <- performance(pred, "tpr", "fpr")
  auc_test <- performance(pred, measure = "auc")@y.values[[1]]
  perf <- performance(pred, measure = "prec", x.measure = "rec")
  perf@y.values[[1]][1] <- perf@y.values[[1]][2]
  ...
  cm_test <- confusionMatrix(factor(as.numeric(predict(gb.k, d.test.imputed, type = "response", n.trees = ntr) >= 0.5) + 1, levels = c(1, 2), labels = c("appendicitis", "noAppendicitis")),  d.test.imputed$DiagnosisByCriteria)
...
########################## Baseline: suspected diagnosis (proposed by doctors)

app.data$SuspectedDiagnosis[!is.na(app.data$DiagnosisByCriteria)]
# AUROC
pred <- prediction(as.numeric(app.data$SuspectedDiagnosis[!is.na(app.data$DiagnosisByCriteria)]) - 1, app.data.dis.imputed$DiagnosisByCriteria)
perf <- performance(pred, "tpr", "fpr")
abline(a = 0, b = 1, col = "gray", lty = 2)
...
# Confusion matrix
(cm <- confusionMatrix(app.data$SuspectedDiagnosis[!is.na(app.data$DiagnosisByCriteria)], app.data$DiagnosisByCriteria[!is.na(app.data$DiagnosisByCriteria)]))
...
########################## Baseline: Alvarado score

app.data$AlvaradoScore[!is.na(app.data$DiagnosisByCriteria)]
# AUROC
pred <- prediction(-app.data$AlvaradoScore[!is.na(app.data$DiagnosisByCriteria)], app.data.dis.imputed$DiagnosisByCriteria)
perf <- performance(pred, "tpr", "fpr")
plot(perf, colorize = TRUE, asp = 1)
abline(a = 0, b = 1, col = "gray", lty = 2)
(auc_base <- performance(pred, measure = "auc")@y.values[[1]])
...
########################## Baseline: Pediatric Appendicitis Score (PAS)

app.data$PediatricAppendicitisScore[!is.na(app.data$DiagnosisByCriteria)]
# AUROC
pred <- prediction(-app.data$PediatricAppendicitisScore[!is.na(app.data$DiagnosisByCriteria)], app.data.dis.imputed$DiagnosisByCriteria)
perf <- performance(pred, "tpr", "fpr")
plot(perf, colorize = TRUE, asp = 1)
abline(a = 0, b = 1, col = "gray", lty = 2)
(auc_base <- performance(pred, measure = "auc")@y.values[[1]])
...
########################## Baseline: partially available criteria (Alvarado + PAS + appendix diameter)

# AUROC
pred <- prediction(1 - ((app.data.dis.imputed$AlvaradoScore >= 4 | app.data.dis.imputed$PediatricAppendicitisScore >= 4) * 1.0) * ((app.data.dis.imputed$AppendixDiameter >= 6.0) * 1.0), app.data.dis.imputed$DiagnosisByCriteria)
perf <- performance(pred, "tpr", "fpr")
plot(perf, colorize = TRUE, asp = 1)
abline(a = 0, b = 1, col = "gray", lty = 2)
(auc_base <- performance(pred, measure = "auc")@y.values[[1]])
...
(aupr_base <- computeArea(perf@x.values[[1]], perf@y.values[[1]]))
# Confusion matrix
(cm <- confusionMatrix(factor(1 - ((app.data.dis.imputed$AlvaradoScore >= 4 | app.data.dis.imputed$PediatricAppendicitisScore >= 4) * 1.0) * ((app.data.dis.imputed$AppendixDiameter >= 6.0) * 1.0), 
                                 levels = c(0, 1), labels = c("appendicitis", "noAppendicitis")), app.data.dis.imputed$DiagnosisByCriteria))
cm$byClass["Specificity"]
cm$byClass["Sensitivity"]
cm$byClass["Pos Pred Value"]
cm$byClass["Neg Pred Value"]
...
npv.rand <- 0
for (i in 1:n_perms) {
  rand_perm <- sample(app.data.dis.imputed$DiagnosisByCriteria, 
                      size = length(app.data.dis.imputed$DiagnosisByCriteria), replace = FALSE)
  cm <- confusionMatrix(rand_perm,  app.data.dis.imputed$DiagnosisByCriteria)
